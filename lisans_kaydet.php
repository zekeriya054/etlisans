...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
		...
		$response = array ('imei'=>$imei,'tel_no'=>$_GET['tel_no'],'okul_kodu'=>$_GET['okul_kodu'],'cevap'=>'1');
    }
	
} else {
    
	$response['cevap']='0';
	$sql = "INSERT INTO kullanici(tel_no, okul_kodu,imei) VALUES ('".$_GET['tel_no']."','".$_GET['okul_kodu']."','".$imei."')";
	if ($conn->query($sql) === TRUE) {
	  $response = array ('imei'=>$imei,'tel_no'=>$_GET['tel_no'],'okul_kodu'=>$_GET['okul_kodu'],'cevap'=>'0');
	} 
